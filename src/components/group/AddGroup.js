...
class AddGroup extends React.Component {
  ...
  addGroupSubmit = () => {
      ...
      axios.post(api.url + api.group, {
        name: this.state.groupName,
        activeStatus: this.state.groupActiveStatus
      }, {"headers" : headers})
      .then(function (response) {
          _this.setState({message : (response.data.message || response.data.errmsg)});
      })
  ...
  updateGroupSubmit = () => {
      ...
      axios.put(api.url + api.group + "/" + _this.state.groupId, {
        name: this.state.groupName,
  ...
  componentDidMount () {
    ...
    if (groupId != null) {
      ...
      axios.get(api.url + api.group + "/" + groupId ,{"headers" : headers})
      .then(function (response) {
        _this.setState({
  ...
  render() {
    ...
    return (
      <div class="topPadding">
        ...
        <FormControl aria-describedby="enter first name" error ={!this.state.groupName} className={classes.formControl}>
          <InputLabel htmlFor="groupname-error">Group Name</InputLabel>
          <Input id="groupname-error" value={this.state.groupName} onChange={this.handleChange} name="groupName"/>
          {!this.state.groupName?
          <FormHelperText id="groupname-error-text">Enter Group name</FormHelperText>
          :''}
        </FormControl>
        ...
        <span>
        {this.state.mode === "add" ?
          <Button variant="outlined" color="primary" className={classes.button} onClick={this.addGroupSubmit}>
            Create Group
          </Button>
          :
          <Button variant="outlined" color="primary" className={classes.button} onClick={this.updateGroupSubmit}>
            update Group
          </Button>
